...
import cn.myth.mybatis.reflection.wrapper.*;
...
/**
 * 元对象
 * 有了反射器、元类、对象包装器以后，在使用对象工厂和包装工厂，就可以组合出一个完整的元对象操作类
 * 因为所有的不同方式的使用，包括：包装起策略、包装工厂、统一的方法处理，这些都需要一个统一的处理方，也就是我们的元对象进行管理
 */
